...
geo_resp.raise_for_status() # Crash on bad HTTP
geo_data = geo_resp.json()
if not geo_data.get('results'):
return jsonify({"error": "No location found"}), 404

loc = geo_data [0]['location']
lat = loc lng = loc fema_url = f"https://hazards.fema.gov/arcgis/rest/services/public/NFHL/MapServer/28/query?geometry={lng},{lat}&geometryType=esriGeometryPoint&inSR=4326&spatialRel=esriSpatialRelIntersects&outFields=FLOOD_ZONE,SFHA_TF&f=json"
fema_resp = requests.get(fema_url)
fema_resp.raise_for_status()
fema_data = fema_resp.json()
features = fema_data.get('features', [ 0]['attributes']
